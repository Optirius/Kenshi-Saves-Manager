...
namespace KenshiSavesManager.Helpers
{
    public static class GoogleDriveHelper
    {
        ...
        public static async Task<Dictionary<string, string>> ListSavedGamesAsync(DriveService service)
        {
            var listRequest = service.Files.List();
            listRequest.Spaces = AppDataFolder;
            listRequest.Fields = "files(id, name)";
            var files = await listRequest.ExecuteAsync();
            // The file name includes the .zip extension, so we remove it for the dictionary key.
            return files.Files.ToDictionary(f => Path.GetFileNameWithoutExtension(f.Name), f => f.Id);
        }
